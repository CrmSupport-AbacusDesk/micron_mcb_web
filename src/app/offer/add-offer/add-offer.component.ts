...
export class AddOfferComponent implements OnInit {
  ...
  getCountryList(){
    this.loading_list = false;
    this.db.get_rqst('', 'app_master/getCountry')
    .subscribe( d => {  
      this.loading_list = true;  
      console.log( d );
      this.country_list = d.country;
    });
  }
  ...
  allCtry(){
    console.log(this.addOffer.allCountry);
    
    if( !this.addOffer.allCountry ){
          for (let i = 0; i < this.country_list.length; i++) {
            this.country_list[i].selected = false;
            }
    }else{
        for (let i = 0; i < this.country_list.length; i++) {
              this.country_list[i].selected = true;
        }
     }
  ...
  // allState(){
  //   console.log(this.addOffer.allStates);
  ...
  this.addOffer.karigar_edit_id = this.offer_id;
    ...
    this.db.insert_rqst( { 'offer' : this.addOffer, 'area' : this.country_list, 'gift' : this.giftList , 'login_id' :this.db.datauser.id }, 'offer/add')
    .subscribe( d => {
