...
export class AssignCouponCodeComponent implements OnInit {
  ...
  getKariger(user_type,e) 
  {
      ...
      this.db.post_rqst(  {"filter":{"search":e,'limit':this.limit,'user_type':user_type}}, 'karigar/karigarListCoupon')
      .subscribe( d => {
  ...
  assignCoupon(){
    this.savingData = true;
      ...
      this.db.post_rqst( {"qr_code": this.data.coupon_code,"karigar_id": this.data.karigar_id,'login_id':this.db.datauser.id,'user_type':this.data.user_type }, 'app_karigar/karigarCoupon')
      .subscribe( d => {
